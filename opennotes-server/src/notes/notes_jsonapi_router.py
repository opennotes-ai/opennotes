...
note_filter_builder = (
    ...
    .add_custom_filter(
        ...
        lambda values: not_(
            exists(
                ...
                .where(Rating.rater_id.in_(values))
    ...
    .add_custom_filter(
        "rated_by",
        lambda value: Note.id.in_(select(Rating.note_id).where(Rating.rater_id == value)),
    )
)
    ...
    try:
        ...
        if filter_rated_by_not_in:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="filter[rater_id__not_in] must contain valid UUIDs",
                )
