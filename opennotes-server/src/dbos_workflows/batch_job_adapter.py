...
class BatchJobDBOSAdapter:
    ...
    def __init__(
        ...
        db_session_factory: Callable[
            [], AsyncSession | AbstractAsyncContextManager[AsyncSession]
        ],
    ) -> None:
    ...
    async def _get_session(self) -> AsyncGenerator[AsyncSession, None]:
        ...
        if hasattr(result, "__aenter__"):
            async with result as session:  # type: ignore[union-attr]
                yield session
        else:
    ...
    async def create_for_workflow(
        ...
        try:
            ...
            logger.error(
                "Failed to create BatchJob for workflow",
                extra={
                    "workflow_id": workflow_id,
                    "job_type": job_type,
                    "error": str(e),
    ...
    async def update_status(
        ...
        try:
            async with self._get_session() as db:
                ...
                status_value = (
                    status.value if isinstance(status, BatchJobStatus) else status
                )
