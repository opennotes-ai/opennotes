...
def _determine_scan_status(
    ...
    if (
        ...
        reason = "zero-message scan never observed all_transmitted"
    elif messages_scanned > 0 and finalization_incomplete:
        status = BulkScanStatus.FAILED
        reason = "finalization incomplete after all_transmitted handoff"
    elif messages_scanned > 0 and processed_count == 0 and total_errors > 0:
        status = BulkScanStatus.FAILED
        reason = "100% of messages had errors"
    elif messages_scanned > 0 and processed_count == 0 and error_count > 0:
        status = BulkScanStatus.FAILED
        reason = "all messages had batch errors"
    elif (
        messages_scanned > 0
        and processed_count == 0
        ...
        status = BulkScanStatus.FAILED
        reason = "orchestrator timed out with zero messages processed"
