...
class DistributedHealthCoordinator:
    ...
    async def _update_instances_set(self) -> None:
        try:
            ...
            instance_key = f"{self.INSTANCE_HEALTH_PREFIX}{self.instance_id}"
            await redis_client.client.sadd(self.INSTANCES_KEY, self.instance_id)  # pyright: ignore[reportGeneralTypeIssues]

            instances = await redis_client.client.smembers(self.INSTANCES_KEY)  # pyright: ignore[reportGeneralTypeIssues]
            active_instances = []
