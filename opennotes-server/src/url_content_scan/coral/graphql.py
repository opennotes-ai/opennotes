"""Async Coral GraphQL client used by URL content scan extraction."""
...
from src.url_content_scan.coral.render import render_to_markdown
...
def _safe_request_body(
    ...
    *,
    ...
    return {"query": CORAL_GRAPHQL_QUERY, "variables": {"url": url, "first": first, "after": after}}
    ...
    for _ in range(max_attempts):
        try:
            ...
            if response.status_code in _RETRY_STATUS:
                raise CoralFetchError(f"coral graphql retryable response: {response.status_code}")
...
async def fetch_coral_comments(
    ...
    *,
    ...
    """Fetch Coral comments from `/api/graphql` and render them to Markdown."""
    if page_size < 1:
    ...
    try:
        ...
        raise CoralUnsupportedError(f"coral graphql unsafe endpoint: {exc.reason}") from exc
