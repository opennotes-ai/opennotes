...
class TestCacheKeyGeneration:
    ...
    def test_short_key_not_hashed(self) -> None:
        ...
        key = manager.generate_key("prefix", "short", "key")
    ...
    def test_long_key_is_hashed(self) -> None:
        ...
        manager = CacheManager.__new__(CacheManager)
        long_arg = "x" * 150
        key = manager.generate_key("prefix", long_arg)
    ...
    def test_kwargs_included_in_key(self) -> None:
        ...
        manager = CacheManager.__new__(CacheManager)
        key = manager.generate_key("prefix", "arg1", foo="bar", baz="qux")
    ...
    def test_kwargs_sorted_for_consistency(self) -> None:
        ...
        manager = CacheManager.__new__(CacheManager)
        key1 = manager.generate_key("prefix", foo="1", bar="2", zeta="3")
        key2 = manager.generate_key("prefix", zeta="3", foo="1", bar="2")
        assert key1 == key2

    def test_empty_args_returns_prefix_only(self) -> None:
        """Empty args should return just the prefix."""
        manager = CacheManager.__new__(CacheManager)
        key = manager.generate_key("prefix")
        assert key == "prefix"
    ...
    def test_long_key_with_kwargs_uses_xxhash3(self) -> None:
        ...
        manager = CacheManager.__new__(CacheManager)
        long_arg = "y" * 50
        key = manager.generate_key("prefix", long_arg, extra="z" * 60)
