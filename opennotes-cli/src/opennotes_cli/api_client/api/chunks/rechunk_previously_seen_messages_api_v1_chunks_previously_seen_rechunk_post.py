...
def _get_kwargs(
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/v1/chunks/previously-seen/rechunk",
        "params": params,
    }
...
def sync_detailed(
    *,
    ...
    """Re-chunk and re-embed previously seen messages
     ...
     Initiates a background task to re-chunk and re-embed previously seen messages for the specified
    community. Useful for updating embeddings after model changes or migration to chunk-based
    embeddings. Requires admin or moderator access to the community server. Rate limited to 1 request
    per minute. Returns 429 if operation already in progress for this community.

    Args:
        community_server_id (UUID): Community server ID for filtering and LLM credentials
