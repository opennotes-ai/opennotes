...
| Custom | Any platform you build using the adapter contract |
...
<Note>
  The `community_server_id` you set in your adapter must be stable. Changing it after you have existing notes or ratings will break historical linkage.
</Note>
...
For Discourse this is typically the Discourse site URL or a short stable slug. For Discord it is the guild's snowflake ID. Document it in your adapter's configuration.
...
Use the `GET /api/public/v1/community-servers/lookup` endpoint to resolve a platform community server ID to its OpenNotes UUID:
