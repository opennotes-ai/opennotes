...
<Note>
  Adapter keys use the scope `platform:adapter`. This scope is distinct from user-grantable scopes. Only community owners with `platform_admin` status can mint `platform:adapter` keys. The platform.opennotes.ai dashboard handles this automatically for registered communities.
</Note>
...
- `POST /api/public/v1/moderation-actions` — record adapter-side moderation actions
