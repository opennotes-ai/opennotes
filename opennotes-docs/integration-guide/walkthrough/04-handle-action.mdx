...
After the request from [Step 3](/integration-guide/walkthrough/03-submit-request) is scored and reaches community consensus, Open Notes produces a **moderation action** — a structured decision telling your integration what to do with the post. Your integration receives this decision in one of two ways:
...
If you have registered a webhook endpoint (see [Webhooks](/integration-guide/concepts/webhooks)), Open Notes will POST a payload to your URL when a consensus decision is reached. The payload looks like this:
