---
title: Webhooks
description: How OpenNotes delivers moderation decisions back to your adapter — delivery semantics, HMAC signing, ordering, and the polling fallback.
---

After OpenNotes scores content and reaches a consensus, it delivers the decision to your adapter via a **webhook**. Your adapter registers an HTTPS endpoint; the server posts a signed JSON payload to that URL when a decision is ready.
...
OpenNotes does not guarantee strict ordering across different `request_id` values. For a single `request_id` you will receive at most one `moderation.decision` event. If your adapter needs total ordering, buffer events and sort by `_webhook_timestamp`.

## Polling fallback

If your adapter is temporarily unable to receive webhooks (downtime, firewall changes), use polling to catch up:
...
- [Adapter contract](/integration-guide/concepts/adapter-contract) — full adapter responsibilities
