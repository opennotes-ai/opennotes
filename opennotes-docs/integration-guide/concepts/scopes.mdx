...
`platform:adapter` is the scope every external adapter needs. It signals that the caller is a trusted adapter speaking on behalf of a community. Without it, the server will not read `X-Adapter-*` headers for user identity — they are simply ignored.
...
<Tabs>
  <Tab title="Adapter scopes">
    These are the scopes an external adapter typically uses:
