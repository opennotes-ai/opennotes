...
description: Authorization bearer token + X-Adapter-* headers end-to-end — how every adapter request is authenticated and how user identity flows through the system.
---

Every call your adapter makes to `/api/public/v1` must carry an `Authorization` header and a set of `X-Adapter-*` headers. This page explains what each header does, what happens when one is missing or wrong, and how the server distinguishes external adapters from internal services.

## The two-layer auth model

```
Adapter ──────────────────────────────────────────► OpenNotes API
...
These headers carry the acting user's platform identity. They are validated only if the API key has the `platform:adapter` scope — the scope gate prevents unauthorized header injection.
...
The plugin uses `X-API-Key` for the key (a legacy header the server also accepts). New adapters should prefer `Authorization: Bearer`.

## X-Platform-* headers — internal only

`X-Platform-*` headers carry signed internal service identity. They are **stripped from all external requests** by `InternalHeaderValidationMiddleware` before any route handler runs. External adapters cannot send these headers to impersonate platform-level identity.

<Note>
  You will never need to set `X-Platform-*` headers in an external adapter. They are only used by internal OpenNotes services that share the `INTERNAL_SERVICE_SECRET`.
</Note>
