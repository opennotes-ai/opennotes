...
All plugin settings live at **Admin > Settings > Plugins > OpenNotes**.

The plugin implements the [adapter contract](/integration-guide/concepts/adapter-contract) — the
...
<Tabs>
  <Tab title="Connection">
    ...
    | `opennotes_server_url` | Base URL of your OpenNotes server, e.g. `https://api.opennotes.ai`. The plugin appends `/api/public/v1/...` to this value for every API call. |
...
<Tabs>
  <Tab title="Automated actions">
    ...
    | `opennotes_route_flags_to_community` | `true` | Forward user flags on monitored posts to OpenNotes for community review, in addition to the normal staff flag queue. |
  </Tab>
...
Per-category settings are configured in the category admin panel under the **OpenNotes** tab.
...
<Note>
  Make sure your Discourse instance is reachable from your OpenNotes server on the webhook
  endpoint. If you run behind a firewall, allow inbound requests from the OpenNotes server IP
  range.
</Note>
