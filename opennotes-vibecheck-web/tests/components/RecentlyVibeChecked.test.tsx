...
describe("<RecentlyVibeChecked />", () => {
  ...
  it("does not render a FeedbackBell in gallery items", () => {
    const analyses = [
      makeAnalysis({
        job_id: "job-bell-check",
        page_title: "Bell Check Article",
        source_url: "https://example.com/bell-check",
      }),
    ];

    render(() => <RecentlyVibeChecked analyses={analyses} />);

    expect(screen.queryByLabelText(/Send feedback about/i)).toBeNull();
  });
