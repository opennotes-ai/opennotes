 ...
 * configured subsections transitions running → done. Allow up to 180s for the
 * whole pipeline. Poll the DOM rather than asserting on intermediate
 * timing — only the terminal state is load-bearing.
 */
test("AC1: golden-path Quizlet URL completes all sections within 180s", async ({
  page,
}) => {
  ...
  expect(
    doneCount,
    `All ${ALL_SECTION_SLUGS.length} sections must reach 'done' (got ${doneCount}: ${JSON.stringify(finalStates)})`,
