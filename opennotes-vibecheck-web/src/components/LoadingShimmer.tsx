...
export default function LoadingShimmer(props: LoadingShimmerProps) {
  ...
  return (
    ...
    >
      <div class="h-6 w-2/3 animate-pulse rounded-md bg-muted" />
        ...
        <For each={Array.from({ length: rowCount() })}>
          {() => (
            <div class="rounded-md border border-border bg-card p-4">
              ...
              <div class="mt-3 h-3 w-full animate-pulse rounded bg-muted" />
              <div class="mt-2 h-3 w-5/6 animate-pulse rounded bg-muted" />
            </div>
          )}
        </For>
      </div>
