...
3. Browser-submitted HTML from the extension is sanitized with `strip_for_display` before storage and markdown fallback generation (`src/routes/scrape.py:289`). PDF extraction keeps model-input sanitation separate with `strip_for_llm` (`src/jobs/pdf_extract.py:45`).
